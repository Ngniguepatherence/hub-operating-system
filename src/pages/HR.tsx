...
export default function HR() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'active':
        return <Badge className="bg-success/20 text-success border-0">{t('common.active')}</Badge>;
      case 'on_leave':
        return <Badge className="bg-warning/20 text-warning border-0">{t('hr.onLeave')}</Badge>;
      case 'inactive':
        return <Badge className="bg-muted text-muted-foreground border-0">{t('common.inactive')}</Badge>;
      default:
        return null;
    }
  };
